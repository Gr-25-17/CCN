    ...
    const wireStateForms = () => {
        document.addEventListener('submit', async (event) => {
            ...
            const response = await fetch(form.action, {
                method: 'POST',
                headers: token ? { RequestVerificationToken: token, 'X-Requested-With': 'XMLHttpRequest' } : { 'X-Requested-With': 'XMLHttpRequest' },
                body: data
            });
            ...
            if (statusCell) statusCell.innerHTML = isDeleted ? '<span class="badge rounded-pill bg-danger">Borttagen</span>' : '<span class="badge rounded-pill bg-success">Aktiv</span>';
            ...
            const targetId = getTargetTableId(row, isDeleted);
            const targetBody = document.querySelector(`table[data-table-id="${targetId}"] tbody`);
